...
I bolded the part about vocals sounding the way they would to the person who's singing them while editing this paragraph because I thought it was kind of funny, and maybe meaningful. It sounds like it's coming from inside the house!
...
I pulled the data using the Spotify API, which I accessed using the `spotifyr` package in R. I used the `get_artist_audio_features` and `get_track_audio_analysis functions` function to acquire data on Lorde, Taylor Swift, St. Vincent, Lana Del Rey, The Chicks, Florence and the Machine, and Bleachers discographies. I threw in HAIM, Marina and the Diamonds, Maggie Rogers, Sharon Von Etten, and Mitski in too, since they've never collaborated with Jack Antonoff but are more or less part of the same pop space.
...
I joined these data sets using the `rbind` function in base R, and created an additional variable called "jack", which contains a 1 if Jack Antonoff produced or co-produced the song, and a 0 if he had nothing to do with it. I got the information on which songs Antonoff did or did not produce from the "Jack Antonoff Production Discography" Wikipedia page \[cite\].
...
I thanked Glen for this time, ended the call, and booted up the ❤️statistical computing programming language, R ❤️\[citeR\].
...
It looks like Jack-produced songs err towards danceability, across the spectrum of energy. It's a bit harder to gauge any trends from the valence quadrant. These quadrants are a start, but they don't suggest any defining Jack-iness characteristic underlying each of those blue points, which are scattered pretty broadly.
...
P(y=1\|x) is the probability of the dependent variable *Y* being 1, given the independent variables *x1*, *x2*, \..., *xn*, and *b0*, *b1*, *b2*, \..., *bn.* The function e\^(-(b0 + b1*x1 + b2*x2 + \... + bn\*xn)), also known as the logit function, is the exponential function of the linear combination of the independent variables and coefficients. βn represents the weights assigned to each independent variable xn. The product of βn and Xn is the contribution of the xn variable to the predicted outcome.

\[Figure X\] gives us an idea of the mean value of each of these features for Jack and non-Jack song. A logistic regression will give us a better idea of how to weigh each of these features in order to predict the outcome, which gives us information about the "research" "question": are these characteristics related to Jack's music? (And then: Which ones? And then, the most elusive question: How?)
...
After some minor pre-processing using the `tidyverse` and `lubridate` R packages. I followed Julia Silge's tutorial \[TK\] to build a model that deals with the imbalanced classes.

I used the `tidymodels` to fir the model. To deal with the imbalanced classes, I downsampled the majority class using the **`step_downsample()`** function from the **`themis`** package. Then I used 5-fold cross-validation with stratification by the **`jack`** variable to estimate the performance of the model.
...
And here are the different variables' importance on determining whether a song was had any Antonoff involvement.
...
Table X shows us the 20 songs in Jack Antonoff's discography with the highest danceability scores. Bleachers takes up 35% of this list, and Taylor Swift takes up 40% of this list. St Vincent, The Chicks, Lorde, and Florence and the Machine also haves songs up there.
...
### In which I do a close listen of our model's top 5 most danceable songs[^4]:

[^4]: I encourage you to follow along and drop me an email with your own evaluation at michaela.drouillard\@mail.utoronto.ca.
