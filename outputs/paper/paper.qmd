...
To find an answer to this question, I turned to statistics, people who know more than me about describing music, and the overlord of engineered virality itself -- the Spotify API. Can Spotify's audio features dataset help us determine what makes Antonoff music so Antonoff-y? Using a logistic regression model, I was able to determine that Spotify's `danceability` feature was the most important in predicting whether or not a song had any Antonoff involvement. During interview with Glen McDonald, an engineer at Spotify, I learned that the `valence` and `danceability` variables were created by hiring college interns to tag whether or not songs were positive or danceabile. In the discussion section, I armchair a bit about what I think this `danceability` feature, which has so much uncertainty cooked into it, is really capturing: visceral-ness. Which validates everyone's seemingly visceral reaction to new Antonoff releases, and the popularity of his music.
